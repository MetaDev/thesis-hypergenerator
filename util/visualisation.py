...
def draw_1D_2D_GMM_variable_sampling(gmmvar,ax=None):
    ...
    for (index0,index1),var in zip(indices,vis_vars):
        ...
        if index1 - index0 == 1:
            ...
            visualise_gmm_marg_1D_density(gmmvar.gmm.marginalise(index1),index0,ax=ax)
        elif index1-index0 == 2:
            #visualise the sibling var range
            draw_2D_stoch_variable(var,ax)
            visualise_gmm_marg_2D_density(gmmvar.gmm.marginalise(index1),np.arange(index0,index1),ax=get_ax(ax,"new"))
        else:
